                        ...
                        {favorites.map((favorite, index) => (
                            <div key={index} className="cart-item">
                                <img src={favorite.image} alt="gown" className="gown-image" onClick={() =>
                                    navigate(`../models/${favorite.model}`)} />
                                <div className="gown-details">
                                    <span className="gown-model">דגם: {favorite.model}</span>
                                    <span className="gown-info">צבע: {favorite.color}</span>
                                    <span className="gown-info">עונה: {favorite.season}</span>
                                    <span className="gown-info">אורך: {favorite.length}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                </> : <h3>אין לך עדיין מועדפים</h3>
            )}
        </div>
    );
}
