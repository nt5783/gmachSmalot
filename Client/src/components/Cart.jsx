...
const Cart = () => {
    ...
    return (
        <div className="cart-container">
                        ...
                        {cart.items.map((gown, index) => (
                            <div key={index} className="cart-item">
                                <img src={gown.img} alt="gown" className="gown-image" onClick={() =>
                                    navigate(`../models/${gown.model}`)} />
                                <div className="gown-details">
                                    <span className="gown-model">{gown.model}</span><br />
                                    <span className="gown-info">גודל: {gown.size}</span><br />
                                    <span className="gown-info">כמות: {gown.qty}</span><br />
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {!user && (
                <h3><a className='login-link' href='./login'>התחברו</a> כדי לנהל את העגלה שלכם</h3>
            )}
            {cart.qty > 0 && <Button
                label="קנה עכשיו"
                icon="pi pi-credit-card"
