...
export default function AddGown({ gowns, model, formOn, getGowns }) {
    ...
    async function addSize(event) {
        ...
        if (newSize && !sizes.find((size) => size.size === newSize)) {
            try {
                ...
                alert(`שגיאה בהוספת גודל: ${err.message}`)
            }
        }
        setAdditional('');
    }
    ...
    async function addGown(data) {
        ...
        if (existingSize != null) {
            if (!window.confirm(`ישנן שמלות מדגם ${model} בגודל ${existingSize}. האם ברצונך להוסיף עוד ${data.amount} יחידות?`)) {
                formOn('');
                return;
            }
        }
        ...
        try {
            ...
            alert(`שגיאה בהוספת שמלה: ${err.message}`)
        }
    }
    ...
    return (
        <>
            <Dialog visible={true} onHide={() => formOn('')} className="add-gown-dialog" header='הוספת גודל חדש לדגם זה'>
                <form onSubmit={handleSubmit((data) => addGown(data))} className="add-gown-form">
                    <br />
                    <div className="field">
                    ...
                    <div className="field">
                        ...
                        <Button label="הוסף גודל" className="p-button-secondary" onClick={() => setAdditional(prev => prev === 'sizes' ? '' : 'sizes')} />
                    </div>
                    <div className="field">
                        <span className="p-float-label">
                            <InputText type="number" id="amount" {...register("amount", { required: true, min: 1 })} min={1} />
