...
function Models() {
    ...
    function modelsHeader() {
        return (<div className='models-header'>
            <div>Models</div>
        ...
        return (
            <Card className='model_item' onMouseOut={() => setShowModelDetails(null)} onMouseOver={() => setShowModelDetails(model.model)} onClick={() => navigate(`./${model.model}`)}>
                <div className='model-icons'>
                    ...
                    {user && !favorites.some((m) => m.model == model.model) && <span className='pi pi-star' onClick={(event) => { event.stopPropagation(); setFavorites((prev) => [...prev, model]) }} />}
                    {user && favorites.some((m) => m.model == model.model) && <span className='pi pi-star-fill' onClick={(event) => { event.stopPropagation(); setFavorites((prev) => prev.filter(m => m.model != model.model)) }} />}
                </div>
                <h3>{model.model}</h3>
        ...
        <Panel header={modelsHeader}
            ...
            {date != null ? (
                ...
                (<div className="warning">Pay attention that the gown may not be available for the date of your event.
                    <br />To view only models with gowns available for the date of your event <a className="no-background" href="./eventCalendar">pick a date here</a>
                </div>)}
