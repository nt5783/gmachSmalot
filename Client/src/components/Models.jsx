...
function Models() {
    ...
    async function getModels() {
        try {
            ...
            if (err.status != 404)
                alert(`שגיאה בקבלת המודלים: ${err.message}`)
        }
    }
    ...
    async function deleteModel(model) {
        if (confirm('האם אתה בטוח שברצונך למחוק את המודל הזה מהבסיס נתונים?')) {
            try {
                fetchNoParamsfunc(`models/${model}`, 'DELETE')
                const index = models.findIndex(m => m.model === model);
                setModels((prev) => [...prev.slice(0, index), ...prev.slice(index + 1, prev.length)])
                alert(`המודל ${model} נמחק בהצלחה\n מעכשיו, המודל הזה לא יוצג יותר`)
            } catch (err) {
                alert(`שגיאה במחיקת המודל: ${err.message}`)
            }
        }
    }
    ...
    const itemTemplate = (model) => {
        ...
        return (
            <Card className='model_item' onMouseOut={() => setShowModelDetails(null)} onMouseOver={() => setShowModelDetails(model.model)} onClick={() => navigate(`./${model.model}`)}>
                ...
                <img height={400} src={model.image} alt={model.model} />
                <div>
                    {showModelDetails == model.model && <span>צבע: {model.color}, עונה: {model.season}, אורך: {model.length}</span>}
                </div>
            </Card>
        );
    };
    ...
    function modelsHeader() {
        return (
            <div className='models-header'>
                ...
                {user && user.isManager === 1 && (
                    <div className='manager-model-options'>
                        <Button className='manager-button' label="הוסף מודל חדש" icon="pi pi-plus" onClick={() => setAddUpdateModelForm(0)} />
                    </div>)}
            </div>
        )
    }
    ...
    return (
        <div className="models-page">
            ...
            <Panel header={modelsHeader}
                ...
                {date != null ? (
                    <div className="event-info">
                        <p>המודלים עם שמלות זמינות לאירוע שלך: <b>{eventDate.getDate()}/{eventDate.getMonth() + 1}/{eventDate.getFullYear()}. </b>
                            <a className="no-background" href="./eventCalendar">שנה תאריך</a> או <a href="#" onClick={() => handleClearDate()}> נקה תאריך</a></p>
                    </div>
                ) : (
                    <>
                        <div className="warning">שימו לב שהשמלה עשויה לא להיות זמינה לתאריך האירוע שלכם.
                            <br />כדי לראות רק מודלים עם שמלות זמינות לתאריך האירוע שלכם <a className="no-background" href="./eventCalendar">בחרו תאריך כאן</a></div>
                    </>
                )}
                {models.length > 0 && <div className="filter-by"></div>}
                <DataView value={models} itemTemplate={itemTemplate} layout="grid" />
