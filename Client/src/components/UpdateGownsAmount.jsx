...
export default function UpdateGownsAmount({ gown, getGowns, index, formOn }) {
    ...
    async function updateGown(data) {
        try {
            ...
            alert(`שגיאה בעדכון שמלה: ${err.message}`)
        }
    }

    return (
        <Dialog header='עדכון כמות' visible={true} onHide={() => formOn('')} className="update-gown-dialog">
            <form onSubmit={handleSubmit((data) => updateGown(data))} className="update-form">
                <br />
                <div className="field">
                    <label><b>מידה: {gown.size}</b></label>
                </div>
                <br />
                <div className="field">
                    <span className="p-float-label">
                ...
                <p>שימו לב: ייתכן שיש הזמנות עתידיות לשמלות במידה זו, הפחתת הכמות לא תשפיע על הזמנות אלו.</p>
                <Button type="submit" label="החל שינויים" className="p-button-success" />
            </form>
        </Dialog>
    );
}
