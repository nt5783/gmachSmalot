...
function Manager() {
    ...
    async function showOrders(when) {
        try {
            ...
            alert(`שגיאה בקבלת ההזמנות: ${err.message}`);
        }
    }
    ...
    return (
        <>
            <div>
                {visible && <div style={{ background: "green" }}>{message}</div>}
                <Button onClick={() => showOrders('today')}>הצג הזמנות של היום</Button>
                <Button onClick={() => showOrders('past')}>הצג את כל ההזמנות הקודמות</Button>
                <Button onClick={() => showOrders('future')}>הצג את כל ההזמנות העתידיות</Button>
                {orders.length === 0 && <h2>אין הזמנות</h2>}
                {orders.length > 0 &&
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            {orders.map((order, i) => (
                                <tr className='view-order' key={i}>
                                    <td><button><i className='pi pi-edit'></i>ערוך</button></td>
                                    <td>{new Date(order.eventDate).toISOString().substring(0, 10)}</td>
                                    <td>{order.fullName}</td>
                                    <td>{order.email}</td>
                                    <td>{order.phone}</td>
